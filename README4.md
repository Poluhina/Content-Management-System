...
В адмике WordPress: 
- добавлено 5 заметок с разными приоритетами
  
- установлены даты напоминания с помощью метабокс
  
- проверено, что каждая заметка сохраняется корректо и отображается в списке заметок

<img width="1250" height="377" alt="image" src="https://github.com/user-attachments/assets/4748e11f-605e-410a-b014-2c043debe9c3" />

2) Проверка отображения и стилей

- все заметки отображаются в оформленных блоках

- заголовок, текст, приоритет и дата заметки видны пользователю
...
Пользовательская таксономия это способ группировки записей. Она создает отдельную сущность, с помощью которой записи можно классифицировать и фильтровать на сайте.

Метополе это дополнительные данные, которые прикрепленны к определенной записи. Метаданные хранят инфформацию о записи, но не создают отдельной категории.

Как пример, таксономия используется если нужно фильтровать или сортировать списки заметок с одинаковым приоритетом. (High, Medium, Low)

Меаполе хранит уникальные значения для каждой заметки и спользуется для сортировке по дате (due_data), но не для группировки по категориям. 

Получается, таксономию использовать когда нужно классифицировать записи и дать пользователю возможность фильтровать их. А метаполя использовать для индивидуальных данных каждой записи.

2) Зачем нужен nonce при сохранении метаполей и что произойдёт, если его не проверять?

Этот уникальный токе защищает формы от атак злоумышленников которые отправляют запросы от имени пользователя без его ведома. При добавлении метаполя создается nonce и вставляет в форму. 

Если не проверять nonce, тогда любой пользователь или злоумышленник может отправить POST-запрос  и измннить данные записи. Это создает угрозу для безопасности сайта. 

В итоге,  nonce гарантирует что изменения метаполя происходит только через форму от администратора.
...
- ```public``` определяет, будет ли тип записей доступен на сайте для пользователей.

Важно для UX, чтобы нужные записи отображались на фронтенде.

- ```support``` какие функции поддерживает СPT: title, editor, author.

Влияет на интерфейс в админке и отображение на сайте.

- ```menu_icon``` иконка в админке.

Улучшает UX администратора при работе с большим количеством типов записей.
